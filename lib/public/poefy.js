...
// Flash the DOM element to draw attention.
...
function getPoemFromAPI(options, callback) {
  if (!requestInProgress) {
    ...
    $.getJSON("poem?" + $.param(options), function(data) {
      ...
      // If there's an error, set the text to the error text.
      // Also, flash the associated DOM element.
      if (data.error) {
        text = data.error.text;
        switch (data.error.type) {
          case "Poefy::MissingFormOrRhyme":
            flashElement("#dropdown_poetic_forms");
            flashElement("#text_rhyme");
            break;
          case "Poefy::RhymeError":
            flashElement("#text_rhyme");
            break;
          case "Poefy::SyllableError":
            flashElement("#text_syllable");
            break;
          case "RegexpError":
          case "Poefy::RegexError":
            flashElement("#text_regex");
            break;
          case "Poefy::MissingDatabase":
            flashElement("#dropdown_databases");
            break;
          case "Poefy::NotEnoughData":
            break;
        }

      // If there's no error.
      } else {
        text = data.poem.join("<br>").replace(/ /g, "&nbsp;");
      }
